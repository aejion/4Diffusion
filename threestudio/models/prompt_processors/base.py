...
class PromptProcessorOutput:
    ...
    def get_text_embeddings(
        ...
        if view_dependent_prompting:
            ...
            text_embeddings = self.text_embeddings.expand(batch_size * 8, -1, -1)  # type: ignore
...
class PromptProcessor(BaseObject):
    ...
    def get_image(self, image_path: str = None):
        if image_path is None:
            return None
        images = []
        for i in range(self.cfg.image_num):
            frame_idx = int(self.cfg.total_num / self.cfg.image_num * i)
            if 'rgba' in image_path:
                path = image_path.replace('0_rgba.png', '{0}_rgba.png'.format(frame_idx))
            else:
                path = image_path.replace('0.png', f"{frame_idx}.png")
            image = Image.open(path)
            image = image.resize((256,256))
            images.append(image)
            print('load prompt image from:', path)
        return images
