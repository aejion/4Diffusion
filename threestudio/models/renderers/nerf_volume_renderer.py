        ...
        **kwargs
        ...
        if self.training:
            geo_out = self.geometry(
                positions, output_normal=self.material.requires_normal or self.need_normal
            )
        ...
        if self.training:
            tv_loss = self.geometry.encoding.encoding.tv_loss(positions)
    ...
    def update_step(
        ...
        def occ_eval_fn(x):
            ...
            if dynamic:
                ...
                timestamps_list = torch.linspace(0.0, 1.0, 8)
                density = torch.zeros(x.shape[0], 1).float().to(x.device)
                for timestamp in timestamps_list:
                    # print(timestamp.item())
                    encoding.frame_time = timestamp.item()
                    encoding.update_occ_grid = True
                    density = torch.max(density, self.geometry.forward_density(x))
            else:
